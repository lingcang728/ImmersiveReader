 ...
 * 而不是误报「没有发现任何回答或文章」。
 */
async function waitForRealContent(page: Page, shouldStop?: () => 'paused' | 'cancelled' | null): Promise<void> {
  const deadline = Date.now() + 20000;
  let reloaded = false;

  while (Date.now() < deadline) {
    // P2-27②：质询等待窗口最长 20s，期间每轮都检查暂停/取消，随时可被叫停。
...
export async function scrapePeopleIndex(
  ...
  // P2-27②：调用方传入的任务停止检查（暂停/取消），让长索引滚动在半秒内响应。
  shouldStop?: () => 'paused' | 'cancelled' | null
  ...
  // P2-27⑥：监听器必须成对摘除 —— 登录墙/人机验证/分页不完整等错误路径曾经在
  // page.off 之前直接 throw，同一 page 反复索引会累积监听器（泄漏 + MaxListeners 告警）。
  ...
  try {
    ...
    //     等待期间每轮都检查暂停/取消（P2-27②）。
    await waitForRealContent(page, shouldStop);
    ...
    let stoppedByUser: 'paused' | 'cancelled' | null = null;
    ...
    while (!stopReason && scrollCount < maxScrolls) {
      stoppedByUser = shouldStop?.() ?? null;
      if (stoppedByUser) break;
      await page.evaluate(() => window.scrollTo(0, document.body.scrollHeight));
      // P2-27②：2-3s 的滚动等待切成 <=500ms 的轮询片，暂停/取消在半秒内生效，
      // 不再要把最长 ~10min 的索引滚动周期跑完才响应。
      const waitDeadline = Date.now() + 2000 + Math.random() * 1000;
      while (Date.now() < waitDeadline) {
        stoppedByUser = shouldStop?.() ?? null;
        if (stoppedByUser) break;
        await sleep(Math.min(500, Math.max(0, waitDeadline - Date.now())));
      }
      if (stoppedByUser) break;
      scrollCount++;
    ...
    if (stoppedByUser) {
      // 用户主动暂停/取消：部分结果照常返回（已发现条目经 onProgress 增量落库），
      // 绝不能按 PAGINATION_INCOMPLETE 抛错 —— 那是抓取异常语义，不是用户停止（P2-27②）。
      notify(`索引 ${itemType} 已${stoppedByUser === 'paused' ? '暂停' : '取消'}：已发现 ${collected.size} 条，恢复时会重新扫描。`);
      const partial = Array.from(collected.values());
      return topN === null ? partial : partial.slice(0, topN);
    }
